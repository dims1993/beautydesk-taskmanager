import React from "react";
import { LogOut } from "lucide-react";
import { APP_MAIN_NAV_TABS } from "./navTabs";

const MobileNavbar = ({
  activeTab,
  setActiveTab,
  onLogout,
  currentUser,
  guidedTourActive = false,
}) => {
  const fiscalIncomplete =
    String(currentUser?.role || "").toUpperCase() === "OWNER" &&
    currentUser?.organization_id == null;

  return (
    <nav
      id="app-nav-mobile"
      className={`fixed bottom-[max(2rem,env(safe-area-inset-bottom,0px))] left-1/2 -translate-x-1/2 md:hidden w-auto ${guidedTourActive ? "z-[210]" : "z-50"}`}
    >
      <div className="flex items-center gap-2 px-3 py-2 bg-white/40 backdrop-blur-md rounded-full border border-white/40 shadow-[0_8px_32px_rgba(93,80,69,0.1)]">
        {APP_MAIN_NAV_TABS.map((tab) => {
          const Icon = tab.icon;
          const isActive = activeTab === tab.id;

          return (
            <button
              key={tab.id}
              type="button"
              data-nav-tab={tab.id}
              title={
                tab.id === "ajustes" && fiscalIncomplete
                  ? "Ajustes — datos fiscales"
                  : tab.title
              }
              onClick={() => setActiveTab(tab.id)}
              className={`relative flex items-center justify-center w-10 h-10 rounded-full transition-all duration-500 ${
                isActive
                  ? "bg-[var(--bt-primary)] text-white shadow-md"
                  : "text-[var(--bt-primary)] opacity-40"
              } ${
                tab.id === "ajustes" && fiscalIncomplete && !isActive
                  ? "ring-2 ring-amber-400/90"
                  : ""
              }`}
            >
              <Icon size={16} strokeWidth={isActive ? 2.5 : 1.5} />

              {isActive && (
                <span className="absolute -bottom-1 w-1 h-1 bg-[var(--bt-primary)] rounded-full animate-pulse" />
              )}
            </button>
          );
        })}

        <div className="w-[1px] h-4 bg-[var(--bt-primary)]/10 mx-1" />

        <button
          type="button"
          title="Salir"
          onClick={onLogout}
          className="flex items-center justify-center w-10 h-10 rounded-full text-[var(--bt-muted)] opacity-40 hover:opacity-100 transition-opacity"
        >
          <LogOut size={16} strokeWidth={1.5} />
        </button>
      </div>
    </nav>
  );
};

export default MobileNavbar;
